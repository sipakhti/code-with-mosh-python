...
def to_pickle(filename,obj,overwrite=True):
    """ A function that saves the object to a pickle file.
        filename is the complete path to the file.
        obj is the object that you want to be saved.
        overwrite is default argument.
        if overwrite is set to False, the function will check for an existing file,
        if present it will ask the user to overwrite the existing file
        OR to create a new file with '_new' appended at the end of the file name
        
        if overwrite set to True then it will always overwrite the file without prompt
    """
    if path.exists(filename) and overwrite == False:
        print(f"The file, [{filename}], already exists ")
        user_in = input("Press Enter to overwrite the eisting file or press N to create a new file: ")
        if user_in = "":
            pass

        else:
            filename.replace(".pkl","_new.pkl")
    
    with open(filename,"wb") as f_obj:
        pickle.dump(obj,f_obj,-1)
        print("File saved")


def from_pickle(filename):
    """ a simple function to load data from a pickle file
        filename is the complete path to the file.
    """
    try:
